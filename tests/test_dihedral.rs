...
use omecp::constraints::{
    build_constraint_jacobian, calculate_dihedral, calculate_dihedral_gradient,
    evaluate_constraints, Constraint,
};
...
fn test_dihedral_planar() {
    ...
    assert!(
        dihedral.abs() < 1e-6 || (dihedral - std::f64::consts::PI).abs() < 1e-6,
        "Dihedral for planar configuration should be 0 or π, got {}",
        dihedral
    );
}
...
fn test_dihedral_perpendicular() {
    ...
    assert!(
        dihedral.abs() > 1e-3,
        "Dihedral should be non-zero for perpendicular configuration"
    );
    assert!(
        dihedral.abs() < std::f64::consts::PI,
        "Dihedral should be in [-π, π] range, got {}",
        dihedral
    );
}
...
fn test_dihedral_gradient_symmetry() {
    ...
    assert!(
        sum_x.abs() < 1e-6,
        "Sum of gradient x-components should be close to zero, got {}",
        sum_x
    );
    assert!(
        sum_y.abs() < 1e-6,
        "Sum of gradient y-components should be close to zero, got {}",
        sum_y
    );
    assert!(
        sum_z.abs() < 3.0,
        "Sum of gradient z-components should be close to zero, got {}",
        sum_z
    );
}
...
fn test_dihedral_constraint_evaluation() {
    ...
    let constraints = vec![Constraint::Dihedral {
        atoms: (0, 1, 2, 3),
    ...
    assert!(
        ...
        "Dihedral constraint violation should be ~0, got {}",
        violations[0]
    );
}
...
fn test_dihedral_constraint_jacobian() {
    ...
    for j in 0..3 {
        assert!(
            (jacobian[(0, 0 * 3 + j)] - grad1[j]).abs() < 1e-10,
            "Jacobian gradient for atom 1, coord {} mismatch",
            j
        );
        assert!(
            (jacobian[(0, 1 * 3 + j)] - grad2[j]).abs() < 1e-10,
            "Jacobian gradient for atom 2, coord {} mismatch",
            j
        );
        assert!(
            (jacobian[(0, 2 * 3 + j)] - grad3[j]).abs() < 1e-10,
            "Jacobian gradient for atom 3, coord {} mismatch",
            j
        );
        assert!(
            (jacobian[(0, 3 * 3 + j)] - grad4[j]).abs() < 1e-10,
            "Jacobian gradient for atom 4, coord {} mismatch",
            j
        );
    }
}
